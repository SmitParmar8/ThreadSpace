...
function renderPosts(posts: any[]): void {
...
function renderAiPosts(posts: any[]): void {
...
function attachPostHandlers(): void {
  // Vote buttons
  ...
  document.querySelectorAll('.tag-chip').forEach(chip => {
    chip.addEventListener('click', (e) => {
      const tagId = parseInt((e.currentTarget as HTMLElement).dataset.tag!);
      document.querySelectorAll('.tag-filter-btn').forEach(b => b.classList.remove('active'));
  ...
  document.querySelectorAll('.delete-btn').forEach(btn => {
    btn.addEventListener('click', async (e) => {
      const postId = (e.currentTarget as HTMLElement).dataset.id!;
...
function openEditModal(postId: number): void {
  ...
  (document.getElementById('editTitle') as HTMLInputElement).value = post.title;
  (document.getElementById('editContent') as HTMLTextAreaElement).value = post.content;

  const tagSelect = document.getElementById('editTag') as HTMLSelectElement;
  tagSelect.innerHTML = allTags.map(t =>
    `<option value="${t.id}" ${t.id === post.tag?.id ? 'selected' : ''}>${t.name}</option>`
  ).join('');
...
function updateNewPostBtn(activeTab: string): void {
...
document.getElementById('saveEditBtn')?.addEventListener('click', async () => {
  ...
  const content = (document.getElementById('editContent') as HTMLTextAreaElement).value.trim();
  const tagId = parseInt((document.getElementById('editTag') as HTMLSelectElement).value);
